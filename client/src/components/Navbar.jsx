...
import bandera_es from "../imgs/es.svg"
import bandera_en from "../imgs/us.svg"
import cv from "../imgs/CV-JuanPabloMas.pdf"
import cv2 from "../imgs/CV-JuanPabloMas-ENG.pdf"
...
function Navbar() {
  ...
  function descargarArchivo(url, url2){
    const nombreArchivo1 = url.split("/").pop();
    const aTag = document.createElement("a");
    aTag.href = url;
    aTag.setAttribute("download", nombreArchivo1);
    document.body.appendChild(aTag);
    aTag.click();
    aTag.remove();
    const nombreArchivo2 = url2.split("/").pop();
    const bTag = document.createElement("a");
    bTag.href = url2;
    bTag.setAttribute("download", nombreArchivo2);
            ...
            <nav className={`${clicked ? "open-nav" : ""}`}>
              ...
              <div className="idioma">
                <div onClick={() => i18n.changeLanguage("es")} className="idioma-item" data-language='es'>
                  <img src={bandera_es} alt="Español" width="30px" />
                </div>
                <div onClick={() => i18n.changeLanguage("en")} className="idioma-item" data-language='en'>
                  <img src={bandera_en} alt="English" width="30px" />
                </div>
              </div>
            </nav>
            <div onClick={() => {descargarArchivo(cv, cv2)}} className="cv-link">
